...
export const Option = {
  ...
  flatMap: <A, B>(f: (a: A) => Option<B>) => (a: Option<A>): Option<B> => Option.isSome(a) ? f(a) : undefined,
  flatMapErr:
    <A, B, Err extends Error>(f: (a: A) => br.Result<B, Err>, err: () => Err) => (a: Option<A>): br.Result<B, Err> =>
  ...
  toResult: <A, E extends Error>(err: () => E) => (a: Option<A>): br.Result<A, E> =>
    Option.isSome(a) ? br.Result.ok(a) : br.Result.err(err()),
  toResultOption: <A, E extends Error>(err: () => E) => (a: Option<A>): br.Result<Option<A>, E> =>
    Option.isSome(a) ? br.Result.ok(a) : br.Result.err(err()),
  filter: <A>(f: (a: A) => boolean) => (a: Option<A>): Option<A> => Option.isSome(a) && f(a) ? a : undefined,
};
...
export const Tuple = {
  map: <A, B>(f: (a: A) => B) => ([a, b]: [A, A]): [B, B] => [f(a), f(b)],
};
...
export const Result = {
  zip: resultZip,
  map: <A, B, Err extends Error>(f: (a: A) => B) => (result: br.Result<A, Err>): br.Result<B, Err> => result.map(f),
  flatMap:
    <A, B, Err extends Error>(f: (a: A) => br.Result<B, Err>) => (result: br.Result<A, Err>): br.Result<B, Err> =>
      result.chain(f),
  validate:
    <A, Err extends Error>(f: (a: A) => boolean, err: () => Err) => (result: br.Result<A, Err>): br.Result<A, Err> =>
      result.chain(a => (f(a) ? br.Result.ok(a) : br.Result.err(err()))),
  unwrapOr: <A>(def: A) => <Err extends Error>(result: br.Result<A, Err>): A => {
