...
export const parseSquare =
    (rules: Rules) =>
        (str: SquareName | string): Square | undefined => {
            const { files } = dimensionsForRules(rules);
...
export const parseUci =
    (rules: Rules) =>
        (str: string): Move | undefined => {
