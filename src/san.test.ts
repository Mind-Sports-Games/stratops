...
const makeSan = makeSanRules('chess');
const parseUci = parseUciRules('chess');

test('make variation with king move', () => {
  const pos = Chess.default();
  const variation = 'e2e4 e7e5 e1e2'.split(' ').map(uci => parseUci(uci)!);
  expect(sanVariation(pos, variation)).toBe('1. e4 e5 2. Ke2');
  expect(pos).toEqual(Chess.default());
});

test('make crazyhouse variation', () => {
  const setup = parseFen('chess')('r4b1N~/1ppk1P2/p1b5/6p1/8/1PBPPq2/P1PR1P2/1K4N1/PNBRPPPrqnn b - - 71 36').unwrap();
  const pos = Crazyhouse.fromSetup(setup).unwrap();
  const variation = 'N@a3 b1b2 R@b1'.split(' ').map(uci => parseUci(uci)!);
  expect(sanVariation(pos, variation)).toBe('36... N@a3+ 37. Kb2 R@b1#');
  expect(pos).toEqual(Crazyhouse.fromSetup(setup).unwrap());
});

test('make stockfish line with many knight moves', () => {
  const setup = parseFen('chess')('2rq1rk1/pb1nbp1p/1pn3p1/3pP3/2pP4/1N3NPQ/PP3PBP/R1B1R1K1 w - - 0 16').unwrap();
...
test('make en passant', () => {
  const setup = parseFen('chess')('6bk/7b/8/3pP3/8/8/8/Q3K3 w - d6 0 2').unwrap();
  const pos = Chess.fromSetup(setup).unwrap();
...
test('parse antichess', () => {
  ...
  for (const san of line) pos.play(parseSan(pos, san)!);
  expect(makeFen('chess')(pos.toSetup())).toBe('8/2k5/8/8/8/8/6b1/8 w - - 0 32');
});
