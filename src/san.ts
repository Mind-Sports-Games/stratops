...
import { FILE_NAMES, RANK_NAMES, isDrop, Move, CastlingSide, Rules } from './types.js';
...
const makeSanWithoutSuffix =
  (rules: Rules) =>
  (pos: Position, move: Move): string => {
    let san = '';
    if (isDrop(move)) {
...
export const makeSanVariation =
  (rules: Rules) =>
  (pos: Position, variation: Move[]): string => {
    pos = pos.clone();
    const line = [];
...
export const parseSan =
  (rules: Rules) =>
  (pos: Position, san: string): Move | undefined => {
    const ctx = pos.ctx();
